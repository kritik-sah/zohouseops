...
import { RequestSupplyProps, SupplyItem } from "@/types/supply";

const RequestSupply: React.FC<RequestSupplyProps> = (props) => {
  const [searchInput, setSearchInput] = useState<string>("");
  const [addedSupply, setAddedSupply] = useState<SupplyItem[]>([
    {
  ...
  const searchSupply = (e: React.ChangeEvent<HTMLInputElement>) => {
    setSearchInput(e.target.value);
  };

  const updateCount = (itemId: number, newCount: number) => {
    if (newCount !== 0) {
      const updatedSupply = addedSupply.map((item: SupplyItem) => {
        if (item.id === itemId) {
      ...
      const updatedSupply = addedSupply.filter((item: SupplyItem) => {
      ...
      >
        ...
        >
          <div className="absolute top-1/2 -translate-y-1/2 left-0">
            <div className="h-8 w-8 relative" onClick={props.onClose}>
              <Image
          ...
          <h2 className="font-bold text-zo-primary !font-space-grotesk w-[80%] text-center">
